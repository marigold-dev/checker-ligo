...
[@inline] let ptr_of_avl_ptr (ptr: avl_ptr) = match ptr with AVLPtr r -> r

let node_tok (n: node) : tok =
  match n with
  | Leaf leaf -> leaf.value.contents.tok
  | Branch branch -> tok_add branch.left_tok branch.right_tok
  | Root _ -> (failwith internalError_NodeTezFoundRoot : tok)

let node_height (n: node) : nat =
  match n with
  | Leaf _ -> 1n
  | Branch branch ->
    let max = if gt_nat_nat branch.left_height branch.right_height
      then branch.left_height else branch.right_height in
    add_nat_nat max (1n)
  | Root _ -> (failwith internalError_NodeHeightFoundRoot : nat)

[@inline] let node_parent (n: node) : ptr =
  match n with
  | Leaf leaf -> leaf.parent
  | Branch branch -> branch.parent
  | Root _ -> (failwith internalError_NodeParentFoundRoot : ptr)

[@inline] let node_branch (n: node) : branch =
  match n with
  | Branch branch -> branch
  | _ -> (failwith internalError_NodeBranchFoundNonBranch : branch)

[@inline] let node_leaf (n: node) : leaf =
  match n with
  | Leaf leaf -> leaf
  | _ -> (failwith internalError_NodeLeafFoundNonLeaf : leaf)
...
let deref_avl_ptr (mem: mem) (p: avl_ptr): ptr option * auction_outcome option =
  ...
  | Root p -> p
  | _ -> (failwith internalError_DerefAvlPtrFoundNonRoot : ptr option * auction_outcome option)

[@inline] let deref_leaf_ptr (mem: mem) (p: leaf_ptr): leaf =
  match p with LeafPtr p -> node_leaf (mem_get mem p)

[@inline] let node_left (n: node) : ptr =
  let b = node_branch n in b.left

[@inline] let node_right (n: node) : ptr =
  let b = node_branch n in b.right

[@inline] let node_set_parent (p: ptr) (n: node) : node =
  match n with
  | Leaf leaf -> Leaf { leaf with parent = p; }
  | Branch branch -> Branch { branch with parent = p; }
  | Root _ -> (failwith internalError_NodeSetParentFoundRoot : node)
...
let update_matching_child
  ...
  | Leaf _ ->
    (failwith internalError_UpdateMatchingChildFoundLeaf : mem)
  | Branch old_branch ->
...
let ref_rotate_left (mem: mem) (curr_ptr: ptr) : mem * ptr =
  let curr =
    ...
    | Branch curr -> curr
    | _ -> (failwith internalError_RefRotateLeftCurrentPtrNotBranch : branch) in

  let right_ptr = curr.right in
  let right =
    match mem_get mem right_ptr with
    | Branch right -> right
    | _ -> (failwith internalError_RefRotateLeftRightPtrNotBranch : branch) in
...
let ref_rotate_right (mem: mem) (curr_ptr: ptr) : mem * ptr =
  let curr =
    match mem_get mem curr_ptr with
    | Branch curr -> curr
    | _ -> (failwith internalError_RefRotateRightCurrentPtrNotBranch : branch) in

  let left_ptr = curr.left in
  let left =
    match mem_get mem left_ptr with
    | Branch left -> left
    | _ -> (failwith internalError_RefRotateRightLeftPtrNotBranch : branch) in
...
let rebalance (mem: mem) (curr_ptr: ptr) : mem * ptr =
  match mem_get mem curr_ptr with
  | Branch branch ->
    if gt_nat_nat (abs (sub_nat_nat branch.left_height branch.right_height)) (1n) then (
      let diff = sub_nat_nat branch.right_height branch.left_height in


      let heavy_child_ptr =
        if lt_int_int diff ((0)) then branch.left else branch.right in
      let heavy_child = match mem_get mem heavy_child_ptr with
        | Branch b -> b
        | _ -> (failwith internalError_RebalanceHeavyChildNonBranchch : branch) in
      let heavy_child_balance =
        sub_nat_nat heavy_child.right_height heavy_child.left_height in

      let (mem, ptr) =
        if lt_int_int diff int_zero then (
          (* (diff < 0) *)
          if leq_int_int heavy_child_balance int_zero then
            (* (hcb <= 0) *)
            (* Left, Left *)
          ...
          if geq_int_int heavy_child_balance int_zero then
            (* (hcb >= 0) *)
            (* Right, Right *)
...
let rec ref_join_rec
  ...
  if lt_nat_nat
      (abs (sub_nat_nat (node_height left) (node_height right)))
      (2n) then
    ...
    let new_join_direction, left_p, right_p, (ptr, to_fix) =
      ...
      if gt_nat_nat (node_height left) (node_height right) then
...
[@inline] let avl_push_back (mem: mem) (root_ptr: avl_ptr) (value: liquidation_slice) =
  avl_push mem root_ptr value Left

(* Pushes the provided value to the front of the queue *)
[@inline] let avl_push_front (mem: mem) (root_ptr: avl_ptr) (value: liquidation_slice) =
...
let rec balance_bottom_up ((mem, curr_ptr): mem * ptr): mem * avl_ptr =
  ...
  | Leaf _ -> (failwith internalError_BalanceBottomUpFoundLeaf : mem * avl_ptr)
  | Branch b ->
...
let ref_del (mem: mem) (ptr: ptr): mem * avl_ptr =
  ...
  | Leaf _ -> (failwith internalError_RefDelParentIsLeaf : mem * avl_ptr)
...
let avl_delete_empty_tree (mem: mem) (ptr: avl_ptr): mem =
  let (r, _) = deref_avl_ptr mem ptr in
  match r with
  | Some _ -> (failwith internalError_AvlDeleteEmptyTreeNonEmptyTree : mem)
...
let rec ref_peek_front (mem, ptr: mem * ptr) : leaf_ptr * leaf =
  ...
  | Root _ -> (failwith internalError_RefPeekFrontFoundRoot : leaf_ptr * leaf)
...
let ref_split_post_processing
  ...
  | Left -> (
      ...
      | None -> (failwith internalError_RefSplitPostProcessingInvariantFailed : mem * ptr option * ptr option)
      | Some right ->
...
let rec ref_split_rec
  ...
  : mem * ptr option * ptr option =
  match mem_get mem curr_ptr with
  | Root _ -> (failwith internalError_RefSplitRecFoundRoot : mem * ptr option * ptr option)
  | Leaf leaf ->
...
[@inline] let avl_tok (mem: mem) (ptr: avl_ptr) : tok =
  let (r, _) = deref_avl_ptr mem ptr in
  match r with
  | Some ptr -> node_tok (mem_get mem ptr)
  | None -> tok_zero

[@inline] let avl_height (mem: mem) (ptr: avl_ptr): nat =
  let (r, _) = deref_avl_ptr mem ptr in
  match r with
  | Some ptr -> node_height (mem_get mem ptr)
  | None -> 0n

[@inline] let avl_root_data (mem: mem) (ptr: avl_ptr) : auction_outcome option =
  let (_, d) = deref_avl_ptr mem ptr in d

[@inline] let avl_modify_root_data (mem: mem) (ptr: avl_ptr) (f: auction_outcome option -> auction_outcome option) =
