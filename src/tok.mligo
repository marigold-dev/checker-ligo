...
[@inline] let tok_add (x: tok) (y: tok) = add_nat_nat x y
let tok_sub (x: tok) (y: tok) =
  match is_nat (sub_nat_nat x y) with
  | Some n -> n
  | None -> (failwith internalError_TokSubNegative : tok)

[@inline] let tok_min (x: tok) (y: tok) = if leq_nat_nat x y then x else y
[@inline] let tok_max (x: tok) (y: tok) = if geq_nat_nat x y then x else y
...
let tok_of_fraction_ceil (x_num: int) (x_den: int) : tok =
  ...
  else abs (cdiv_int_int (mul_int_int x_num tok_scaling_factor_int) x_den)

let tok_of_fraction_floor (x_num: int) (x_den: int) : tok =

  if lt_int_int x_num ((0))
  then (failwith internalError_TokOfFractionFloorNegative : tok)
  else abs (fdiv_int_int (mul_int_int x_num tok_scaling_factor_int) x_den)

[@inline] let tok_scale (amnt: tok) (fp: fixedpoint) =
  tok_of_fraction_floor
    (mul_int_nat (fixedpoint_to_raw fp) amnt)
    (mul_int_int fixedpoint_scaling_factor_int tok_scaling_factor_int)

[@inline] let geq_tok_tok = geq_nat_nat
[@inline] let leq_tok_tok = leq_nat_nat

[@inline] let lt_tok_tok = lt_nat_nat
[@inline] let gt_tok_tok = gt_nat_nat

[@inline] let eq_tok_tok = eq_nat_nat
