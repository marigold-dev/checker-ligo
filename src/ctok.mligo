...
[@inline] let ctok_add (x: ctok) (y: ctok) = add_nat_nat x y
let ctok_sub (x: ctok) (y: ctok) =
  match is_nat (sub_nat_nat x y) with
  | Some n -> n
  | None -> (failwith internalError_CtokSubNegative : ctok)

[@inline] let ctok_min (x: ctok) (y: ctok) = if leq_nat_nat x y then x else y
[@inline] let ctok_max (x: ctok) (y: ctok) = if geq_nat_nat x y then x else y
...
let ctok_of_fraction_ceil (x_num: int) (x_den: int) : ctok =
  ...
  else abs (cdiv_int_int (mul_int_int x_num ctok_scaling_factor_int) x_den)

let ctok_of_fraction_floor (x_num: int) (x_den: int) : ctok =

  if lt_int_int x_num ((0))
  then (failwith internalError_CtokOfFractionFloorNegative : ctok)
  else abs (fdiv_int_int (mul_int_int x_num ctok_scaling_factor_int) x_den)

[@inline] let ctok_scale (amnt: ctok) (fp: fixedpoint) =
  ctok_of_fraction_floor
    (mul_int_nat (fixedpoint_to_raw fp) amnt)
    (mul_int_int fixedpoint_scaling_factor_int ctok_scaling_factor_int)

[@inline] let geq_ctok_ctok = geq_nat_nat
[@inline] let leq_ctok_ctok = leq_nat_nat

[@inline] let lt_ctok_ctok = lt_nat_nat
[@inline] let gt_ctok_ctok = gt_nat_nat

[@inline] let eq_ctok_ctok = eq_nat_nat
