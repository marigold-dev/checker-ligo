...
[@view]
let buy_kit_min_kit (param: Ctok.ctok) (wrapper: CheckerT.wrapper): Kit.kit =
...
[@view]
let sell_kit_min_ctok (param: Kit.kit) (wrapper: CheckerT.wrapper): Ctok.ctok =
  match wrapper.deployment_state with
...
[@view]
let add_liquidity_max_kit (param: Ctok.ctok) (wrapper: CheckerT.wrapper): Kit.kit =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_add_liquidity_max_kit_deposited (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: Kit.kit)

[@view]
let add_liquidity_min_lqt (param: Ctok.ctok) (wrapper: CheckerT.wrapper): Lqt.lqt =
  match wrapper.deployment_state with
...
[@view]
let remove_liquidity_min_ctok (param: Lqt.lqt) (wrapper: CheckerT.wrapper): Ctok.ctok =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_remove_liquidity_min_ctok_withdrawn (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: Ctok.ctok)

[@view]
let remove_liquidity_min_kit (param: Lqt.lqt) (wrapper: CheckerT.wrapper): Kit.kit =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_remove_liquidity_min_kit_withdrawn (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: Kit.kit)

[@view]
let current_auction_details (_: unit) (wrapper: CheckerT.wrapper): CheckerT.view_current_liquidation_auction_details_result =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_current_liquidation_auction_details ((), checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: CheckerT.view_current_liquidation_auction_details_result)

[@view]
let burrow_max_mintable_kit (param: CheckerT.burrow_id) (wrapper: CheckerT.wrapper): Kit.kit =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_burrow_max_mintable_kit (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: Kit.kit)

[@view]
let is_burrow_overburrowed (param: CheckerT.burrow_id) (wrapper: CheckerT.wrapper): bool =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_is_burrow_overburrowed (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: bool)

[@view]
let is_burrow_liquidatable (param: CheckerT.burrow_id) (wrapper: CheckerT.wrapper): bool =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_is_burrow_liquidatable (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: bool)

[@view]
let get_balance (param: (address * FA2.fa2_token_id)) (wrapper: CheckerT.wrapper): nat =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_get_balance (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: nat)

[@view]
let total_supply (param: FA2.fa2_token_id) (wrapper: CheckerT.wrapper): nat =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_total_supply (param, checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: nat)

[@view]
let all_tokens (_: unit) (wrapper: CheckerT.wrapper): FA2.fa2_token_id list =
  match wrapper.deployment_state with
  | Sealed checker -> Checker.view_all_tokens ((), checker)
  | Unsealed _ -> (failwith error_ContractNotDeployed: FA2.fa2_token_id list)

[@view]
let is_operator (param: (address * (address * FA2.fa2_token_id))) (wrapper: CheckerT.wrapper): bool =
  match wrapper.deployment_state with
